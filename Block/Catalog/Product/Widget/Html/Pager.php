...
/**
 * Products by Quantity Range widget pager block
 */
namespace Me\Aos\Block\Catalog\Product\Widget\Html;

use Magento\Theme\Block\Html\Pager as HtmlPager;
use Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection;
use Magento\Catalog\Block\Product\Widget\Html\Pager as WidgetHtmlPager;
...
class Pager extends HtmlPager
{
    ...
    {
        if (null === $this->collectionSize) {
            $this->collectionSize = $this->getCollection()->getSize();
            if ($this->getTotalLimit() && $this->collectionSize > $this->getTotalLimit()) {
                $this->collectionSize = $this->getTotalLimit();
            }
        }
    ...
    public function getCurrentPage()
    {
        if (null === $this->currentPage) {
            ...
            if ($page > $this->getLastPageNum()) {
                $this->currentPage = $this->getLastPageNum();
            } elseif ($page > 0) {
    ...
    public function getLastPageNum()
    {
        if (null === $this->lastPage) {
            $this->lastPage = ceil($this->getCollectionSize() / $this->getLimit());
            if ($this->lastPage <= 0) {
                $this->lastPage = 1;
            }
        }
